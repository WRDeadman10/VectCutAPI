...
def add_video_track(
    ...
    if transition:
        try:
            # Get transition type
            if IS_CAPCUT_ENV:
                transition_type = getattr(draft.CapCut_Transition_type, transition, None) or getattr(draft.CapCut_Transition_type, transition.capitalize(), None) or getattr(draft.CapCut_Transition_type, transition.title(), None)
            else:
                transition_type = getattr(draft.Transition_type, transition, None) or getattr(draft.Transition_type, transition.capitalize(), None) or getattr(draft.Transition_type, transition.title(), None)
    ...
    if mask_type:
        try:
            if IS_CAPCUT_ENV:
                mask_type_enum = getattr(draft.CapCut_Mask_type, mask_type, None) or getattr(draft.CapCut_Mask_type, mask_type.capitalize(), None) or getattr(draft.CapCut_Mask_type, mask_type.title(), None)
            else:
                mask_type_enum = getattr(draft.Mask_type, mask_type, None) or getattr(draft.Mask_type, mask_type.capitalize(), None) or getattr(draft.Mask_type, mask_type.title(), None)
            
            if mask_type_enum is None:
                raise AttributeError()
            ...
            raise ValueError(f"Unsupported mask type {mask_type}, supported types include: Circle, Rectangle, Split, Filmstrip, Stars, Heart. Error: {str(e)}")
